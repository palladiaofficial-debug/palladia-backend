...
// ── GET /api/v1/badge/:code/presence-history — storico timbrature (PUBBLICO) ──
// Stesso modello di accesso di /badge/:code e /badge/:code/punch: il badge_code
// stesso è la credenziale, coerente con tutto il resto di quest'area. Era stato
// rimosso in favore del login CF di workerArea.js, ma il frontend (tab "Storico"
// dell'Area Lavoratore) non è mai stato aggiornato a quel flusso e chiamava
// sempre questa URL — 404 reale in produzione. Ripristinato 2026-07-11.
...
// ── GET /api/v1/badge/:code/payslips — buste paga condivise (PUBBLICO) ────────
// Stesso ripristino di cui sopra: la tab "Buste paga" chiamava una URL che non
// è mai esistita sotto /badge/:code, solo sotto /area/:code (auth CF, mai
// collegata dal frontend). Dati sensibili (stipendio) dietro badge_code, stesso
// livello di protezione già in uso per presenze/timbratura in questo modulo.
router.get('/badge/:code/payslips', badgeLimiter, async (req, res) => {
  const { code } = req.params;
  if (!/^[A-Fa-f0-9]{18}$/i.test(code)) return res.status(400).json({ error: 'INVALID_BADGE_CODE' });
...
// ── GET /api/v1/badge/:code/payslips/:id/pdf — URL firmato PDF (PUBBLICO) ─────
router.get('/badge/:code/payslips/:id/pdf', badgeLimiter, async (req, res) => {
...
// ── POST /api/v1/badge/:code/payslips/:id/acknowledge — presa visione (PUBBLICO) ─
router.post('/badge/:code/payslips/:id/acknowledge', badgeLimiter, async (req, res) => {
  const { code, id } = req.params;
  if (!/^[A-Fa-f0-9]{18}$/i.test(code)) return res.status(400).json({ error: 'INVALID_BADGE_CODE' });
