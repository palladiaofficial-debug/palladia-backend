...
async function _buildBrain(supabase, companyId) {
  ...
  const [workersRes, sitesRes, equipRes, ncRes, workerSitesRes, siteCostsRes, subsRes] = await Promise.allSettled([
    supabase
      .from('workers')
  ...
  const text = _buildText(enrichedWorkers, sites, equipment, deadlines, openNCs, costsBySite, workersNoTariffa, subcontractors);
...
function _buildText(workers, sites, equipment, deadlines, openNCs, costsBySite, workersNoTariffa, subcontractors = []) {
  const now = new Date();
