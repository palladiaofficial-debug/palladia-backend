...
async function _buildBrain(supabase, companyId) {
  ...
  const [workersRes, sitesRes, equipRes, ncRes, workerSitesRes, siteCostsRes] = await Promise.allSettled([
    supabase
      .from('workers')
  ...
  const text = _buildText(enrichedWorkers, sites, equipment, deadlines, openNCs, costsBySite, workersNoTariffa);
...
function _buildText(workers, sites, equipment, deadlines, openNCs, costsBySite, workersNoTariffa) {
  const now = new Date();
