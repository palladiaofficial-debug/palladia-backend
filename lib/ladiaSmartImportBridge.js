...
// a Sonnet (F-050, AUDIT.md — ~50s per 44 pagine/16 buste paga reali).
...
async function startImportFromChatUpload({ uploadId, companyId, userId }) {
  ...
  let items = (await batchItemsSnapshot(result.batchId, companyId)) || [];
  while (Date.now() < deadline) {
    const sum = summarize(items);
    if (!sum.stillProcessing && items.length > 0) break;
    await sleep(POLL_INTERVAL_MS);
    items = (await batchItemsSnapshot(result.batchId, companyId)) || [];
  }
...
async function confirmHighConfidenceBatch({ batchId, companyId, userId, req = null }) {
  ...
  return await pipeline.confirmAllGreen(batchId, companyId, userId, req);
}
