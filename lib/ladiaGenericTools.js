...
async function createRecord(resourceName, rawPayload, companyId, userId, req, opts = {}) {
  ...
  return { success: true, record: data, actionHistoryId, undoSummary: summary };
}
...
async function updateRecord(resourceName, recordId, rawPatch, companyId, userId, req, opts = {}) {
  ...
  return { success: true, record: data, previous: before || {}, actionHistoryId, undoSummary: summary };
}
...
async function deleteRecord(resourceName, recordId, companyId, userId, req, opts = {}) {
  ...
  return { success: true, deleted_id: recordId, actionHistoryId, undoSummary: summary };
}
...
async function undoAction(historyId, companyId, userId) {
  ...
  if (entry.action === 'create') {
    // Conflitto: se il record non esiste più (già cancellato da qualcun altro
    // nel frattempo) non c'è nulla da annullare due volte.
  ...
  await supabase.from('ladia_action_history')
