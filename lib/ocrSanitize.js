...
// Stessa esigenza di sanitizeExtractedFields, per il pattern "lista di
// suggerimenti {label, reason}" usato da baracca.js — un'altra rotta che
// chiede all'IA un array JSON via prompt testuale (nessuno schema imposto) e
// il cui frontend (BaraccaTab.tsx) renderizza label/reason direttamente come
// figli React.
function sanitizeLabelReasonList(list) {
  return (Array.isArray(list) ? list : [])
    .filter((item) => item && typeof item === 'object')
    .map((item) => ({ label: flattenToText(item.label), reason: flattenToText(item.reason) }))
    .filter((item) => item.label);
}

module.exports = { sanitizeExtractedFields, sanitizeLabelReasonList, flattenToText };
