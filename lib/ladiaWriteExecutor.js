...
// Seguito F-118, LADIA_EVALS 2026-09-06 (U09 su produzione): l'id della riga
// appena creata non basta per una tabella-ponte come worksite_workers — il
// suo id (della riga di assegnazione) è diverso dal worker_id che un tool
// alternativo (remove_worker_from_site) userebbe per riferirsi alla stessa
// entità. Estrae anche i valori dei campi del payload originale, per
// bloccare un'azione successiva che referenzia lo STESSO worker/entità, non
// solo la stessa riga. site_id/company_id esclusi di proposito — troppo
// generici (condivisi da qualunque altra scrittura sullo stesso cantiere):
// includerli romperebbe una richiesta composta legittima su un'entità
// diversa nello stesso turno (es. "annulla X e poi fai Y su un altro
// lavoratore/cantiere").
const GENERIC_ID_FIELDS = new Set(['site_id', 'company_id', 'table', '_confirmed']);
function collectBlockCandidates(input, depth = 2) {
  const out = [];
  if (!input || typeof input !== 'object' || depth < 0) return out;
  for (const [k, v] of Object.entries(input)) {
    if (GENERIC_ID_FIELDS.has(k)) continue;
    if (typeof v === 'string') out.push(v);
    else if (v && typeof v === 'object') out.push(...collectBlockCandidates(v, depth - 1));
  }
  return out;
}
...
module.exports = { executeWrite, proposeBespokeAction, checkOrProposeGate, collectBlockCandidates };
