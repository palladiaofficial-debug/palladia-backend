...
// Specchio delle stesse condizioni che undoAction() (ladiaGenericTools.js)
// verifica DAVVERO al momento del click — usato qui solo per decidere se
// mostrare il bottone "Annulla azione" nella card, non per eseguire l'undo.
// Va tenuto in sync con undoAction(): create richiede allow.delete, delete
// richiede allow.create, update è sempre strutturalmente annullabile (i
// controlli su conflitto/snapshot mancante lì si possono fare solo al click).
function isUndoable(resource, action) {
  if (action === 'create') return !!resource.allow.delete;
  if (action === 'delete') return !!resource.allow.create;
  return true;
}
...
function buildSummary(resourceName, action, record) {
  const nameField = ['full_name', 'name', 'nome', 'titolo', 'ragione_sociale', 'descrizione'].find(f => record && record[f]);
...
async function logAction({ companyId, userId, req, conversationId, resourceName, action, recordId, record, previousValues, changedFields, fullRowSnapshot, auditActionOverride }) {
  ...
  if (!resource) {
    ...
    return { actionHistoryId: null, summary: null, resource: resourceName, action, record: record || null, previous: previousValues || null, changedFields: changedFields || null, recordId: recordId ?? null };
  }
  ...
  // changedFields incluso nel return (non solo scritto in ladia_action_history):
  // la card SSE record_action in routes/v1/chat.js legge result.changedFields
  // per mostrare il diff — senza questo campo la card mostrava sempre "nessun
  // diff" per ogni tool bespoke che chiama logAction, anche se l'audit trail
  // e l'undo funzionavano comunque correttamente (leggono da ladia_action_history,
  // non dal valore di ritorno).
  return { actionHistoryId, summary, resource: resourceName, action, record: record || null, previous: previousValues || null, changedFields: changedFields || null, undoable: isUndoable(resource, action), recordId: recordId ?? null };
}

module.exports = { buildSummary, logActionHistory, logAction, isUndoable };
