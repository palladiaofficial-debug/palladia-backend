...
// F-216 (AUDIT.md), seguito: la scadenza reale di pagamento è nello stesso blocco
// DettaglioPagamento già letto per il metodo, ma non veniva mai estratta — una
// fattura fornitore importata (A-Cube, email, importazione massiva) non poteva
// mai contribuire "dati certi" alla previsione di cassa, anche quando il
// documento la dichiarava esplicitamente. Con più righe DettaglioPagamento
// (pagamento rateale) si prende la scadenza più vicina, quella che conta per
// la previsione a breve — non la più lontana, che sottostimerebbe il rischio.
function extractDueDate(datiPagamento) {
  const dettagli = asArray(asArray(datiPagamento)[0]?.DettaglioPagamento);
  const dates = dettagli
    .map((d) => firstOf(d?.DataScadenzaPagamento, null))
    .filter(Boolean)
    .map(String)
    .sort();
  return dates[0] || null;
...
function parseFatturaPaXml(xmlString) {
  ...
  return {
    ...
    dueDate:         extractDueDate(body.DatiPagamento),
