...
if (dsn) {
  Sentry.init({
    ...
    beforeSend(event, hint) {
      ...
      // ECCEZIONE: gli errori Anthropic (APIError) hanno anch'essi .status ma un 400/429
      // lì indica quasi sempre un bug reale (prompt troppo lungo, immagine malformata,
      // rate limit raggiunto) — vanno tracciati, non scartati come i 4xx delle nostre API.
      if (err?.status && err.status < 500 && !(err instanceof APIError)) return null;
