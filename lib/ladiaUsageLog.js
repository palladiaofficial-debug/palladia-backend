...
// SUM lato database (RPC ai_usage_daily_spend, migrazione 164) — non un
// fetch di tutte le righe sommato in Node: quest'ultimo approccio troncava
// silenziosamente a 1000 righe (limite di default PostgREST), sottostimando
// la spesa reale su un giorno di traffico pesante. Vedi F-057, AUDIT.md.
async function getTodayAiSpend() {
  const dayStart = `${todayRomeKey()}T00:00:00.000Z`;
  const { data, error } = await supabase.rpc('ai_usage_daily_spend', { p_since: dayStart });
  if (error) throw new Error('DB_ERROR: ' + error.message);
  const row = Array.isArray(data) ? data[0] : data;
  return { total: Number(row?.total || 0), external: Number(row?.external || 0) };
}
...
// F-057 (AUDIT.md): SUM lato database (RPC ai_usage_monthly_spend, migrazione
// 164), non più un fetch di tutte le righe del mese sommato in Node — quel
// pattern troncava silenziosamente a 1000 righe (limite di default
// PostgREST), sottostimando la spesa reale per qualunque company/periodo con
// più di 1000 chiamate AI nel mese (osservato dal vivo: TEST-LadiaEvals,
// 1783 righe — checkAiBudget avrebbe permesso spesa ben oltre il tetto del
// piano senza mai bloccare). Nessuna company reale è oggi sopra le 900
// righe/mese, ma è lo stesso identico gate che protegge i clienti paganti:
// deve restare corretto anche quando l'uso cresce, non solo oggi.
async function getMonthlyAiSpend(companyId) {
  const startOfMonth = new Date();
  startOfMonth.setUTCDate(1);
  startOfMonth.setUTCHours(0, 0, 0, 0);

  const { data, error } = await supabase.rpc('ai_usage_monthly_spend', {
