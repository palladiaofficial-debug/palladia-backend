...
// F-184 (AUDIT.md): un EXIT annotato da un admin (`POST /presence/:logId/annotate`)
// come glitch tecnico (es. il retry di rete che ha capovolto entrata/uscita
// prima del fix) non deve spezzare la sessione in due — la riga resta per
// sempre in presence_logs (append-only), ma qui viene ignorata insieme
// all'ENTRY di autocorrezione che il lavoratore ha quasi certamente timbrato
// subito dopo, così l'ENTRY originale continua ininterrotta. Effetto zero se
// `log.annotation` non è presente (i chiamanti che non lo passano nella query
// si comportano esattamente come prima — opt-in, non un cambio di default).
function stripAnnotatedGlitches(logsSorted) {
  const out = [];
  let i = 0;
  while (i < logsSorted.length) {
    const log = logsSorted[i];
    if (log.event_type === 'EXIT' && log.annotation) {
      const next = i + 1 < logsSorted.length ? logsSorted[i + 1] : null;
      i += (next && next.event_type === 'ENTRY') ? 2 : 1;
      continue;
    }
    out.push(log);
    i += 1;
...
function pairLogsByDay(logsRaw) {
  const logs = stripAnnotatedGlitches(logsRaw);
  const byDay = new Map();
 ...
 * @param {boolean} [skipDeduction]  F-169 (AUDIT.md): true quando l'admin ha
 *   segnalato esplicitamente "niente pausa oggi" per questo lavoratore/giorno
 *   (presence_lunch_overrides) — la detrazione automatica presume che una
 *   pausa non timbrata sia comunque avvenuta, il che penalizza chi ha
 *   davvero lavorato senza sosta invece di uscire prima. L'override salta
 *   la detrazione e paga la presenza reale.
 ...
 */
function applyLunchBreak(pairs, lunchConfig, skipDeduction = false) {
  if (!lunchConfig || lunchConfig.minutes <= 0 || pairs.length !== 1 || skipDeduction) {
...
// Un lavoratore disattivato non deve mai comparire in un documento ufficiale
// — sempre, in qualunque company. Una fixture di test (prefisso `TEST-`,
// stessa convenzione usata da tutti gli script di questo repo — vedi
// feedback_npm_test_accumulates_prod_fixtures) va esclusa SOLO quando la
// company del report è reale: gli oltre 100 selftest di questo repo creano
// deliberatamente lavoratori "TEST-..." dentro company "TEST-..." dedicate
// per verificare che il report LI INCLUDA — un'esclusione incondizionata
// per prefisso avrebbe rotto quei test (verificato: selftest_presence_
// report_lunch_break.js passava da verde a rosso). Scoperto sul Registro
// Presenze reale di MSCedilizia S.r.l. (2026-09-14): due righe
// "TEST-PilotaGeofence" nel PDF consegnabile, seminate da una sessione di
// verifica live direttamente sulla company reale invece che su una dedicata
// — nessun filtro le escludeva prima di questo fix.
function isTestCompanyName(companyName) {
  return typeof companyName === 'string' && companyName.startsWith('TEST-');
}
function isTestOrInactiveWorker(worker, companyName) {
  return !worker || worker.is_active === false
    || (!isTestCompanyName(companyName) && typeof worker.full_name === 'string' && worker.full_name.startsWith('TEST-'));
}
...
module.exports = {
  ...
  isTestOrInactiveWorker, isTestCompanyName,
};
