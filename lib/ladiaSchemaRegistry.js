...
const RESOURCES = {
  workers: {
    ...
    fields: {
      full_name:     { type: 'string', required: true,  sensitivity: 'low' },
      fiscal_code:   { type: 'string', required: false, sensitivity: 'low' },
      role:          { type: 'string', required: false, sensitivity: 'low' },
      qualification: { type: 'string', required: false, sensitivity: 'low' },
      employer_name: { type: 'string', required: false, sensitivity: 'low' },
      // safety_training_expiry / health_fitness_expiry: deliberatamente ASSENTI.
      // Stessa tabella, ma campi legal-sensitive (D.Lgs 81/2008) — restano su
      // update_worker_expiry (bespoke), vedi Fase 3 del piano.
    },
    ...
    bespokeOnly: { update: ['update_worker_expiry'], delete: [] },
  },
  ...
  sites: {
    ...
    defaultSensitivity: 'low',
    fields: {
      name:            { type: 'string', required: true,  sensitivity: 'low', createOnly: true },
