...
const patchSiteSchema = z.object({
  ...
  // Fascia oraria per la precipitazione (F-199, AUDIT.md, migrations/213):
  // null/"" = nessun filtro (conta la pioggia sulle 24h intere).
  weather_shift_enabled: z.boolean().optional(),
  weather_shift_start:   z.union([z.literal(null), z.literal(''), z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'formato HH:MM')]).optional(),
  weather_shift_end:     z.union([z.literal(null), z.literal(''), z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'formato HH:MM')]).optional(),
  // Soglia caldo (D.L. 107/2026 art.6, migrations/218): null/"" = eredita il default azienda.
  heat_temp_threshold_c: weatherNum(20, 50),
  heat_alert_enabled:    z.boolean().optional(),
