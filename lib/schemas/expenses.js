...
const updateExpenseSchema = z.object({
  ...
  notes:             z.string().max(1000).nullish(),
  // F-215 (AUDIT.md): pagina Economia unificata — "Segna pagata" da un
  // elenco movimenti, stesso trattamento già in uso su site_costs/site_sal_history.
  pagato_il:         z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullish(),
});
