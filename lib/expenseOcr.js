...
async function extractExpenseFromDocument(buffer, mimetype, { companyId, userId = null, callSite = 'expense_scan' } = {}) {
  ...
  return match ? sanitizeExpenseFields(JSON.parse(match[0])) : {};
}

// F-077: nessuno schema imposto sul JSON dell'IA (solo un'istruzione nel
// prompt) — su un documento con intestazione fornitore strutturata l'IA può
// impacchettare un campo testuale come oggetto annidato invece di stringa.
// Passato così com'è a res.json() in routes/v1/expenses.js, un campo-oggetto
// renderizzato come figlio React crasha la pagina (stesso errore #31 di
// F-066). `amount` deve restare numerico (usato in un confronto DB e nel
// form), gli altri campi devono restare stringa o null.
const STRING_FIELDS = ['description', 'supplier', 'invoice_number', 'expense_date', 'category', 'payment_method'];
function sanitizeExpenseFields(obj) {
  if (!obj || typeof obj !== 'object') return {};
  const out = { amount: typeof obj.amount === 'number' && !isNaN(obj.amount) ? obj.amount : null };
  for (const field of STRING_FIELDS) out[field] = typeof obj[field] === 'string' ? obj[field] : null;
  return out;
}

module.exports = { extractExpenseFromDocument, sanitizeExpenseFields };
