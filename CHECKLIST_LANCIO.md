...
- [x] 🟢 Redirect corretto dopo login (torna dove stava andando, non sempre alla dashboard) — **verificato via codice 2026-07-12**: `ProtectedRoute.tsx` salva `?next=` (validato anti-open-redirect in `lib/auth.ts`), `Login.tsx` lo rilegge davvero dopo il login e ci reindirizza, fallback `/dashboard` solo se assente
...
- [x] 🟡 Export presenze funziona e i dati coincidono con quelli a schermo — **verificato dal vivo 2026-07-12**: `GET /sites/:id/export` genera un vero **XLSX multi-foglio** (Cantiere/Lavoratori/Presenze/Subappaltatori), non PDF/CSV come la voce originale ipotizzava — nota per correttezza, non un bug: non esiste altro export dedicato oltre a questo. Testato: `200`, file valido (~9.5KB), content-type XLSX corretto
...
- [x] 🟡🔴 **Bug trovato e corretto 2026-07-12**: il banner compliance del tab Organico (`SiteDetail.tsx`) duplicava la logica di calcolo scadenze invece di riusare `lib/compliance.js` (dichiarato "unica fonte di verità, NON duplicare" nel suo stesso commento) — e lo faceva con un metodo diverso: giorno UTC (`toISOString().slice(0,10)`) invece di mezzanotte locale, rischio concreto di disallineamento vicino a mezzanotte per utenti fuori UTC. Estratta `dateComplianceStatus()` condivisa con la stessa identica logica del backend (data locale, parsing Y/M/D). Frontend commit `ae27e59`
...
- [ ] 🟡 Documenti in scadenza evidenziati correttamente — **verificato via codice 2026-07-12: la funzione non esiste affatto**, non è un bug ma una feature mai costruita — `SiteDocument` (frontend) non ha nemmeno il campo `expiry_date`, sia "Documenti Aziendali" che "Documenti Cantiere" mostrano solo nome/categoria/dimensione/data upload, nessun badge di scadenza. Il meccanismo esiste altrove (Scadenze.tsx, banner Organico) ma non è collegato qui. Non bloccante per il lancio, da valutare come feature futura
...
- [x] 🔴 Messaggio con foto allegata → elaborata correttamente, nessun "immagine troppo pesante" — **verificato dal vivo 2026-07-12**: generata con `sharp` una foto sintetica ma dimensionalmente realistica (4032×3024, 3.68MB — dimensione tipica di una foto smartphone non ridimensionata, sotto il limite `IMAGE_TOO_LARGE` di ~5.25MB), inviata a `/chat/stream` con un account/conversazione di prova isolati: `200`, nessun errore di dimensione nello stream SSE, risposta Claude coerente col contenuto reale dell'immagine (non un canned response — l'ha descritta come "grigia/uniforme", corretto per l'immagine generata), messaggio con foto persistito in `chat_messages.images`. Cleanup completo
...
- [x] 🟡 Cronologia raggruppata per data (Oggi/Ieri/Ultimi 7 giorni) corretta — **verificato via codice 2026-07-12**: `Pal.tsx` (`groupConversations`) raggruppa in Oggi/Ieri/Ultimi 7gg/Questo mese/Prima usando orario locale del browser (non UTC fisso), nessun bug off-by-one trovato
...
- [x] 🟡 Rate limit chat (20 msg/min) — non deve scattare in uso normale — **verificato via codice 2026-07-12**: `chatLimiter` (`middleware/rateLimit.js`) 20/min per company_id, applicato PRIMA della chiamata Anthropic in `routes/v1/chat.js` — un utente bloccato riceve `429` senza consumare crediti; soglia realistica per uso normale
- [x] 🔴 **Corretta questa voce 2026-07-18**: non esiste nessun toast "Nuova versione disponibile" da cliccare — scelta esplicita del 7/7 (`main.tsx`), il reload è automatico e senza conferma appena arriva un deploy, priorità a "tutti sempre sull'ultima versione". **Verificato dal vivo con 2 deploy reali in produzione** (l'utente con Ladia aperta, messaggio in scrittura, account `carpiooricardo@gmail.com`): il reload avviene entro ~1 minuto dal deploy, **il messaggio in scrittura viene perso** (comportamento accettato/voluto, non un bug), **la sessione resta intatta** (nessun logout dopo il reload)
...
- [x] 🔴 Scan QR cantiere da telefono lavoratore → identificazione via CF funziona — **verificato dal vivo 2026-07-12** su produzione (cantiere/lavoratore di prova isolati): `GET /scan/worksites/:id` (pubblico) → dati cantiere corretti; `POST /scan/identify` (pubblico) con CF reale → `200`, `session_token` + `worker_id` corretti. Manca solo la prova fisica con fotocamera telefono (inquadratura QR), il backend del flusso è confermato funzionante
...
- [x] 🟡 Badge PDF lavoratore: dati corretti, QR di verifica funzionante — **verificato dal vivo 2026-07-12**: `GET /workers/:id/badge-pdf` → PDF binario valido (~90KB, con QR timbratura + QR verifica), status 200
- [x] 🟢 Pagina pubblica verifica badge (`/badge/:code`) mostra dati corretti senza login — **verificato dal vivo 2026-07-12**: `GET /badge/:code` senza alcun header di auth → `200`, dati (`full_name`, `badge_code`, stato compliance) corretti e coerenti col lavoratore di test
...
- [x] 🟡 Marketplace corsi: ricerca, filtri — **verificato dal vivo 2026-07-12**: corso di test isolato trovato correttamente con `GET /marketplace/courses?q=...&city=Milano`, ed escluso correttamente quando il filtro città non combacia
- [x] 🟡 Area provider formazione: gestione sessioni, notifiche prenotazioni — **verificato dal vivo 2026-07-12**: creazione sessione (`201`), modifica (`200`), lista prenotazioni ricevute mostra la prenotazione di test, completamento prenotazione (`200`) — ciclo completo con provider/corso/sessione isolati, cleanup completo
- [x] 🟢 Recensioni corso — **verificato dal vivo 2026-07-12**: recensione creata dopo booking `completed` (`201`), doppia recensione sullo stesso booking correttamente rifiutata (`409`)
...
- [x] 🔴 Checkout Stripe: pagamento test completato, piano attivato subito dopo — **verificato dal vivo 2026-07-18**: pagamento reale di 1€ (piano Starter scontato con coupon monouso creato ad hoc, scaduto dopo l'uso) completato su `carpiooricardo@gmail.com`/MSCedilizia. Confermato sia lato utente (notifica Google Wallet) sia lato DB: `stripe_customer_id` e `stripe_subscription_id` popolati per la prima volta, `subscription_plan='starter'`, `subscription_status='active'` — webhook Stripe processato correttamente
...
- [x] 🔴 Paywall: schermata corretta, upgrade e pagamento reale funzionano — **verificato dal vivo 2026-07-18**: pagina `/paywall` mostrata e usata per davvero (non solo controllo via codice), pagamento reale completato, piano sbloccato subito dopo (vedi voce sopra). **Bonus trovato durante il test**: il badge "Più scelto" andava a capo su due righe e si sovrapponeva al titolo del piano sotto, più asimmetria tra le card (quella del piano attivo aveva due elementi impilati invece di uno) — corretto (frontend commit `0d238f5`), riverificato con screenshot reale prima del deploy
- [ ] 🟡 Portale gestione abbonamento (cambio piano, cancellazione) funziona — **sbloccato 2026-07-18**: il pagamento di test ha dato a MSCedilizia il primo `stripe_customer_id` reale della piattaforma, quindi `POST /billing/portal` ora ha i dati che gli servivano. Manca solo il click reale come **owner** (`carpiooricardo@gmail.com`, non l'account admin di test) sul pulsante "Gestisci abbonamento" in Impostazioni — non simulabile senza la sessione owner vera
- [ ] 🟡 Webhook Stripe: verificare nei log che gli eventi arrivino e vengano processati — **controllato 2026-07-12**: nessun evento `[stripe-webhook]` nei log Railway recenti (atteso: zero pagamenti reali completati finora, coerente con "nessun cliente pagante"). Il codice che gestisce gli eventi (`server.js`, `/api/webhooks/stripe`) è verificato via lettura — incluso il fix "risponde 500 su errore così Stripe ritenta" già applicato nell'audit del 2026-07-06. Si chiude da sola al primo pagamento reale: controllare i log in quel momento
...
- [x] 🟢 Rimozione membro team: accesso revocato immediatamente — **verificato dal vivo 2026-07-12**: membro `tech` di test aveva accesso (200) prima; owner lo rimuove (`DELETE /team-members/:userId` → `200`, riga cancellata da `company_users`); stesso JWT del membro rimosso, stessa richiesta, subito dopo → `403`, accesso revocato immediatamente senza bisogno di nuovo login
...
- [x] 🔴 Dopo un deploy, l'app si ricarica correttamente restando sulla sessione loggata — **verificato dal vivo 2026-07-18** (vedi sezione 4, stesso test): nessun toast/bottone "Aggiorna" esiste realmente (rimosso il 7/7 a favore del reload automatico), ma il comportamento reale è confermato: reload entro ~1 minuto, sessione mai persa
...
- [x] 🟡 `cache_read_input_tokens` > 0 dal secondo messaggio in poi (conferma che il prompt caching funziona davvero) — **verificato 2026-07-12 da dati reali** in `ladia_usage_log` (non serve chiamare Ladia di nuovo, i dati esistono già): pattern esatto atteso — primo messaggio di una sessione `cache_creation_tokens=40023, cache_read_tokens=0`, messaggi successivi nella stessa sessione `cache_creation_tokens=0, cache_read_tokens=40023` — il caching funziona davvero, non solo a codice. Dati del 2026-07-09 (stesso giorno del fix TTL 5min→1h), nessun traffico chat più recente da verificare
