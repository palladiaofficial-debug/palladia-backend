...
- [ ] 🟡 Portale gestione abbonamento (cambio piano, cancellazione) funziona — **controllato via codice 2026-07-12, non testabile dal vivo qui**: `POST /billing/portal` (solo owner, `403` altrimenti) richiede `stripe_customer_id` valorizzato (`400 NO_CUSTOMER` altrimenti) e genera una vera Billing Portal Session Stripe. **Nessuna company in produzione ha ancora un `stripe_customer_id`** (zero clienti paganti reali ad oggi) — e la chiave Stripe su Railway è **LIVE**, non test: non ho creato un customer Stripe fittizio per non generare artefatti reali sul dashboard Stripe live. Da chiudere quando ci sarà il primo cliente reale, o forzando un test in modalità Stripe test se disponibile
