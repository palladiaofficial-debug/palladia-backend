 ...
 *
 * Stack:
 *   PDF  → Claude Haiku 4.5 con native PDF document API (fast, cheap, preciso)
 *   Excel → xlsx converte in CSV localmente → Claude Haiku 4.5
 *
 * Haiku 4.5 vs Sonnet 4.6: ~4-5x più veloce, ~10x più economico.
 */
...
Struttura output:
{"nome":"titolo o 'Computo metrico'","voci":[{"tipo":"categoria","codice":"A","descrizione":"DEMOLIZIONI","sort_order":0},{"tipo":"voce","parent_codice":"A","codice":"A.01","descrizione":"Demolizione muratura portante","unita_misura":"m³","quantita":45.5,"prezzo_unitario":85.0,"importo":3867.5,"sort_order":1}]}
...
3. Se prezzo_unitario mancante ma ci sono importo e quantita → calcolalo (importo / q).
...
// ── Processa risposta Haiku ────────────────────────────────────────────────────
function processResponse(response, ctx) {
  const raw = response.content[0].text;
  console.log(`[computoParser/${ctx}] stop_reason:${response.stop_reason} tokens_out:${response.usage?.output_tokens} raw[:150]:`, raw.slice(0, 150));

  if (response.stop_reason === 'max_tokens') {
    throw new Error('Il documento è troppo grande. Prova a dividere il computo in sezioni più piccole o usa il formato Excel.');
  }
...
function extractJson(text) {
  ...
  try {
    ...
    console.warn('[computoParser] JSON.parse fallito:', e.message, '— start:', jsonStr.slice(0, 100));
