...
async function run() {
  await runOne('185_economia_movimenti_registro.sql');
  await runOne('186_economia_movimenti_sync_triggers.sql');
  await runOne('187_economia_movimenti_verify_and_mo.sql');
  console.log('Migrazioni 185-187: BLOCCO 1 — registro unico movimenti economici + subappalti + verifica.');
}
